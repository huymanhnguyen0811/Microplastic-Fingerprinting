...
for (mz in benchmark_mz) {
  idx <- which(combined_df2_grouped$m.z <= (mz + 0.0005) & combined_df2_grouped$m.z >= (mz - 0.0005))
...
#### With Combined GC ICP - TBA (ZeroImputed_Jan 15 2024) 
```{r, echo=FALSE, warning = FALSE, message=FALSE}
hc_df <- gc_icp_list[[1]] %>%
  # filter(., File %notin% "ATDGCMS-Blank-repNA") %>%
  dplyr::select(File,
...
for (ele in 60:98) {
  ...
  if (length(satisfied_sp) < 2) {
    ...
    sig_comp_CO60_98[[paste0("Cut height_", ele)]] <- sp_df %>%
      filter(., adjusted_pvalue_holm < 0.05) %>%
      arrange(adjusted_pvalue_holm)
...
writexl::write_xlsx(x = sig_comp_CO60_98, path = "Wilcoxon_test_Combined GC-ICP_by_Robust Aichitson_HCA clustering result_CO60-98_zeroimputed_15Jan2024.xlsx")
```

#### With Combined HPLC ICP - TBA (ZeroImputed_Jan 15 2024) 
```{r, echo=FALSE, warning = FALSE, message=FALSE}
...
for (ele in 100:130) {
  ...
  if (length(satisfied_sp) < 2) {
    ...
    sig_comp_CO100_130[[paste0("Cut height_", ele)]] <- sp_df %>%
      filter(., adjusted_pvalue_holm < 0.05) %>%
      arrange(adjusted_pvalue_holm)
...
writexl::write_xlsx(x = sig_comp_CO100_130, path = "Wilcoxon_test_Combined HPLC-ICP_by_Robust Aichitson_HCA clustering result_CO100-130_zeroimputed_15Jan2024.xlsx")
```

#### With just GC - TBA (ZeroImputed_Jan 15 2024)
...
writexl::write_xlsx(x = sig_comp_CO1_63, path = "Wilcoxon_test_GC_by_Robust Aichitson_HCA clustering result_CO1-63_zeroimputed_15Jan2024.xlsx")
```
...
#### With just HPLC - TBA (ZeroImputed_Jan 15 2024) 
(Update 18 Jan 2024:) To maximise runtime, this will be run on Huy's Laptop
...
for (ele in 40:64) {
  ...
  sig_comp_CO40_64[[paste0("Cut height_", ele)]] <- sp_df %>%
    filter(., adjusted_pvalue_holm < 0.05) %>%
    arrange(adjusted_pvalue_holm)
...
# writexl::write_xlsx(x = sig_comp_CO65_95, path = "Wilcoxon_test_HPLC_by_Robust Aichitson_HCA clustering result_CO65-95_zeroimputed_15Jan2024.xlsx")
```

#### With just ICP - TBA (ZeroImputed_Jan 15 2024) 
(Update 18 Jan 2024:) To maximise runtime, this will be run on Huy's Laptop
...
for (ele in 0:34) {
  ...
  sig_comp_CO0_34[[paste0("Cut height_", ele)]] <- sp_df %>%
    filter(., adjusted_pvalue_holm < 0.05) %>%
    arrange(adjusted_pvalue_holm)
...
writexl::write_xlsx(x = sig_comp_CO0_34, path = "Wilcoxon_test_ICP_by_Robust Aichitson_HCA clustering result_CO0-34_zeroimputed_15Jan2024.xlsx")
```
...
### 1. With HCA
```{r class.source = 'fold-hide', echo=FALSE, warning = FALSE, message=FALSE}
p1 <- paste0(getwd(), "/Wilcoxon test results/Wilcoxon_test_Combined GC-HPLC_by_Robust Aichitson_HCA clustering result_CO130-159_18Dec2023.xlsx")
p2 <- paste0(getwd(), "/Wilcoxon test results/Wilcoxon_test_Combined GC-HPLC_by_Robust Aichitson_HCA clustering result_CO151-162_06Dec2023.xlsx")
p3 <- paste0(getwd(), "/Wilcoxon test results/Wilcoxon_test_Combined GC-HPLC_by_Robust Aichitson_HCA clustering result_CO161-172_06Dec2023.xlsx")
...
sheet_names3 <- excel_sheets(path = p3)
sus_pol_combined3 <- lapply(sheet_names3, function(sheet) read_excel(path = p3, sheet = sheet))

# Using lapply to get unique values from each data frame
unique_values_list3 <- lapply(sus_pol_combined3, function(df) unique(df$comp))

# Unlisting and then finding unique values across all data frames
unique3 <- unique(unlist(unique_values_list3))

all_unique <- data.frame(comp=unique(c(unique1, unique2, unique3))) %>%
  # get rid off all GC compounds since EF is HPLC
...
for (suspect_mz in unique(sus_scr$`[M+H]+`)) {
  ...
  if (base::identical(idx, integer(0))) {
    ...
    temp$Feature <- all_unique[idx,]$comp
    df_list[[i]] <- temp
    i <- i + 1
  }
}
