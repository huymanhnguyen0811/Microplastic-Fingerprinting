...
for (ele in 130:159) {
  ...
  if (length(satisfied_sp) < 2) {
    ...
    sig_comp_CO130_159[[paste0("Cut height_", ele)]] <- sp_df %>%
...
writexl::write_xlsx(x = sig_comp, path = "Wilcoxon_test_Combined GC-HPLC_by_Robust Aichitson_HCA clustering result_CO130-159_zeroimputed_15Jan2024.xlsx")
```
...
writexl::write_xlsx(x = sig_comp, path = "Wilcoxon_test_Combined GC-ICP_by_Robust Aichitson_HCA clustering result_CO130-159_zeroimputed_15Jan2024.xlsx")
```
...
writexl::write_xlsx(x = sig_comp, path = "Wilcoxon_test_Combined HPLC-ICP_by_Robust Aichitson_HCA clustering result_CO130-159_zeroimputed_15Jan2024.xlsx")
```
...
for (ele in 6:55) {
  ...
  sig_comp_CO0_55[[paste0("Cut height_", ele)]] <- sp_df %>%
    filter(., adjusted_pvalue_holm < 0.05) %>%
    arrange(adjusted_pvalue_holm)
...
writexl::write_xlsx(x = sig_comp, path = "Wilcoxon_test_GC_by_Robust Aichitson_HCA clustering result_zeroimputed_15Jan2024.xlsx")
```
...
After cutoff element [[45]], we dont see anymore significant compound from Wilcoxon test
...
writexl::write_xlsx(x = sig_comp, path = "Wilcoxon_test_HPLC_by_Robust Aichitson_HCA clustering result_CO35-44_zeroimputed_s15Jan2024.xlsx")
```
...
writexl::write_xlsx(x = sig_comp, path = "Wilcoxon_test_ICP_by_Robust Aichitson_HCA clustering result_zeroimputed_15Jan2024.xlsx")
```
...
writexl::write_xlsx(x = all_matching_sus_scr, path = "Wilcoxon_test_HPLC_data_matching_with_EF_Suspect_screening_17Jan2024.xlsx")
```
