...
```{r class.source = 'fold-hide', echo = FALSE, message = FALSE, warning = FALSE}
# Use excel sheets of benchmark day "0, 2, 15, 34" from Final Leachates Benchmarks and Recovery.xlsx to create scatterplot 
...
# Plot scatter and histogram of RT distribution
par(mfrow = c(3, 2))
...
# What is the distribution of RT versus. no of peak in each sample?
hist_ls <- list()
i <- 1
...
For RT <= 6 min, the result is follows:
[1] "RT window 0.1 No. unique compounds that belongs to Benchmark_1 Acetaminophen-d4 : 22"
[1] "RT window 0.1 No. compounds of unique compounds : 28808"
[1] "RT window 0.15 No. unique compounds that belongs to Benchmark_1 Acetaminophen-d4 : 12"
[1] "RT window 0.15 No. compounds of unique compounds : 27662"
[1] "RT window 0.2 No. unique compounds that belongs to Benchmark_1 Acetaminophen-d4 : 6"
[1] "RT window 0.2 No. compounds of unique compounds : 26497"

For RT > 6 min, the result is follows:
[1] "RT window 0.1 No. unique compounds that belongs to Benchmark_2 dTEP : 28"
[1] "RT window 0.1 No. unique compounds that belongs to Benchmark_3 dTPrP : 12"
[1] "RT window 0.1 No. unique compounds that belongs to Benchmark_4 13C-6PPD : 6"
[1] "RT window 0.1 No. unique compounds that belongs to Benchmark_5 dTPP : 11"
[1] "RT window 0.1 No. compounds of unique compounds : 29750"
[1] "RT window 0.15 No. unique compounds that belongs to Benchmark_2 dTEP : 18"
[1] "RT window 0.15 No. unique compounds that belongs to Benchmark_3 dTPrP : 4"
[1] "RT window 0.15 No. unique compounds that belongs to Benchmark_4 13C-6PPD : 5"
[1] "RT window 0.15 No. unique compounds that belongs to Benchmark_5 dTPP : 4"
[1] "RT window 0.15 No. compounds of unique compounds : 25805"
[1] "RT window 0.2 No. unique compounds that belongs to Benchmark_2 dTEP : 12"
[1] "RT window 0.2 No. unique compounds that belongs to Benchmark_3 dTPrP : 2"
[1] "RT window 0.2 No. unique compounds that belongs to Benchmark_4 13C-6PPD : 5"
[1] "RT window 0.2 No. unique compounds that belongs to Benchmark_5 dTPP : 3"
[1] "RT window 0.2 No. compounds of unique compounds : 23563"

```{r class.source = 'fold-hide', echo = FALSE, message = FALSE, warning = FALSE}
# Testing different RT window for 2 parts of the chromatogram ----------
# RT <= 6 min
rt_5_list <- list()
for (i in 1:length(c(0.1,0.15,0.2))) {
  # RT <= 5 min
  rt_5_list[[i]] <- grouping_comp(combined_df2 %>% filter(RT <= 6),
                                  rtthres = c(0.1,0.15,0.2)[i],
                                  mzthres = 0.0005,
                                  type = "HPLCTOFMS")
  
  print(paste0("RT window ", c(0.1,0.15,0.2)[i], " No. unique compounds that belongs to Benchmark_1 Acetaminophen-d4 : ", length(unique((rt_5_list[[i]] %>% 
                                                                          filter(m.z >= (156.0957 - 0.0001) &
                                                                                   m.z <= (156.0957 + 0.0001)))$Feature))))
  print(paste0("RT window ", c(0.1,0.15,0.2)[i], " No. of unique compounds : ", length(unique(rt_5_list[[i]]$Feature))))
}


# RT >6 min
rt_above5_list <- list()
for (i in 1:length(c(0.02,0.03,0.04))) {
  # RT <= 5 min
  rt_above5_list[[i]] <- grouping_comp(combined_df2 %>% filter(RT > 6),
                                       rtthres = c(0.02,0.03,0.04)[i],
                                       mzthres = 0.0005,
                                       type = "HPLCTOFMS")
  
  print(paste0("RT window ", c(0.1,0.15,0.2)[i], " No. unique compounds that belongs to Benchmark_2 dTEP : ", length(unique((rt_above5_list[[i]] %>% 
                                                              filter(m.z >= (198.1723 - 0.0001) &
                                                                       m.z <= (198.1723 + 0.0001)))$Feature))))
  print(paste0("RT window ", c(0.1,0.15,0.2)[i], " No. unique compounds that belongs to Benchmark_3 dTPrP : ", length(unique((rt_above5_list[[i]] %>% 
                                                               filter(m.z >= (246.2568 - 0.0001) &
                                                                        m.z <= (246.2568 + 0.0001)))$Feature))))
  print(paste0("RT window ", c(0.1,0.15,0.2)[i], " No. unique compounds that belongs to Benchmark_4 13C-6PPD : ", length(unique((rt_above5_list[[i]] %>% 
                                                                  filter(m.z >= (305.1956 - 0.0001) &
                                                                           m.z <= (305.1956 + 0.0001)))$Feature))))
  print(paste0("RT window ", c(0.1,0.15,0.2)[i], " No. unique compounds that belongs to Benchmark_5 dTPP : ", length(unique((rt_above5_list[[i]] %>% 
                                                              filter(m.z >= (342.1722 - 0.0001) &
                                                                       m.z <= (342.1722 + 0.0001)))$Feature))))
  
  print(paste0("RT window ", c(0.1,0.15,0.2)[i], " No. of unique compounds : ", length(unique(rt_above5_list[[i]]$Feature))))
}
```
...
# HPLCTOFMS
# Apply different alignment to 2 parts of chromatogram
# RT <= 6 min
combined_df2_grouped_rt1 <- grouping_comp(combined_df2 %>% filter(RT <= 6),
                                      rtthres = 0.1,
                                      mzthres = 0.0005,
                                      type = "HPLCTOFMS")
# RT > 6 min
combined_df2_grouped_rt2 <- grouping_comp(combined_df2 %>% filter(RT > 6),
                                      rtthres = 0.02,
                                      mzthres = 0.0005,
                                      type = "HPLCTOFMS")

# Combine resulting df after compound alignment
combined_df2_grouped <- rbind(combined_df2_grouped_rt1, combined_df2_grouped_rt2)
```
...
for (mz in benchmark_mz) {
  idx <- which(combined_df2_grouped$m.z == mz)
...
### Quality assurance: How many compounds after filtering consistent feature (populate 90 % of that one product type) between different RT windows?

### Quality assurance: How many compounds after 'Removing columns with more than 90% NAs' between different RT windows?
...
# How many compounds appear 90% of the time in only one product type

# Remove columns with more than 90% NAs after the IF conditioning
test <- df_pca %>% 
  purrr::discard(~sum(is.na(.x))/length(.x)* 100 >=90)

# Missing value imputation
for (r in 1:nrow(test)) {
  test[r, which(base::is.na(test[r,]))] <- 0
