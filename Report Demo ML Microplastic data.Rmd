...
newmetal <- metal_list[[1]] %>% tidyr::pivot_wider(names_from = Metal_component, values_from = Values) %>% column_to_rownames(., var = "NewFile")
...
topmetal_list <- list()
i <- 1
for (file in unique(metal_list[[1]]$NewFile)) {
  topmetal_list[[i]] <- metal_list[[1]] %>% filter(., NewFile %in% file) %>% filter(., Metal_component %in% top_metal[[i]])
  i <- i + 1
}
...
ggplot(data = top_metal_plot, aes(x = NewFile, y = Values, fill = Metal_component)) +
