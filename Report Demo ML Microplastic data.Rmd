...
for (i in 1:length(excel_sheets(path = p))) {
  ...
  metal_list[[i]] <- dat %>% 
    select(., -c(File, Type)) %>%
    relocate(NewFile, Suspected_Polymer, Category, Subcategory, .before = 1) %>%
    pivot_longer(., cols = 5:ncol(.), names_to = "Metal_component", values_to = "Values")
}
```

#### 1. What are the Top10 metal with highest Concen. in each sample?

```{r class.source = 'fold-hide', echo = FALSE, message = FALSE, warning = FALSE}
newmetal <- metal %>% column_to_rownames(., var = "File")

# Replace all NA in data frame with 0
newmetal[is.na(newmetal)] <- 0

# Extract Top 10 metal with highest concen for each sample
top_metal <- list()
for (row in 1:nrow(newmetal)) {
  top_metal[[row]] <- colnames(newmetal)[order(as.numeric(newmetal[row,]), decreasing = TRUE)[1:10]]
}

# top10_metal <- bind_cols(top10_metal)
# colnames(top10_metal) <- rownames(newmetal)
topmetal_list <- list()
i <- 1
...
ggplot(data = top_metal_plot, aes(x = NewFile, y = Values, fill = Metal_component)) +
  ...
  labs(title = "Stacked Bar Plot", x = "Sample", y = "Value",
       fill = "Top 10 trace metal components") +
