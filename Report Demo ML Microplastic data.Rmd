...
for (r in 1:nrow(df_pca)) {
  df_pca[r, which(base::is.na(df_pca[r,]))] <- 0 # no different in PCA biplot between LOD and zero imputed even with CLR-transformation
    # stats::runif(length(which(base::is.na(df_pca[r,]))),
    #                                                         min = sort(gc_list[[1]]$Values)[1]/100000,
    #                                                         max = sort(gc_list[[1]]$Values)[2]/100000)
}

clr_transformed_pca <- compositions::clr(df_pca)

res.pca <- FactoMineR::PCA(df_pca, # clr_transformed_pca
                           scale.unit = FALSE,
                           graph = FALSE)
...
(Update 04 Feb 2024:) Most common theme in comparison PCA biplot of zero versus. LOD imputation is that Zero imputation provides better higher percentage of data variation explained by PC1 and PC2 than LOD imputation.
...
for (r in 1:nrow(df_pca)) {
  df_pca[r, which(base::is.na(df_pca[r,]))] <- 0
  # stats::runif(length(which(base::is.na(df_pca[r,]))),
  #                                                           min = sort(hplc_list[[1]]$Values)[1]/100000,
  #                                                           max = sort(hplc_list[[1]]$Values)[2]/100000)
}

clr_transformed_pca <- compositions::clr(df_pca[, -1])

res.pca <- FactoMineR::PCA(clr_transformed_pca, # df_pca[, -1], 
                           scale.unit = FALSE,
                           graph = FALSE)
...
factoextra::fviz_pca_biplot(res.pca,  
                            ...
                            repel = TRUE,
                            labelsize = 6,
                            habillage = factor(df_pca$Category),
                            addEllipses = TRUE,
                            ellipse.level=0.95,
                            ggtheme = ggplot2::theme_minimal(base_size = 15)
