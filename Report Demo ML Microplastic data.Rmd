...
#### 1. Top10 metal with highest Concen. in each sample? aka. Significant metals to distinguish between each sample
```{r class.source = 'fold-hide', echo = FALSE, message = FALSE, warning = FALSE}
p <- "C:/Users/huyng/OneDrive - Toronto Metropolitan University/Microplastic/Microplastic-Fingerprinting/data/Table of product categorization_NewFileName.xlsx"
...
for (row in 1:nrow(newmetal)) {
  top10_metal[[row]] <- colnames(newmetal)[order(as.numeric(newmetal[row,]), decreasing = TRUE)[1:10]]
}
...
for (file in unique(metal_list[[1]]$NewFile)) {
  top10_list[[i]] <- metal_list[[1]] %>% filter(., NewFile %in% file) %>% filter(., Metal_component %in% top10_metal[[i]])
  i <- i + 1
}

top10_plot <- bind_rows(top10_list)
...
ggplot(data = top10_plot, aes(x = NewFile, y = Values, fill = Metal_component)) +
...
For combined GC/HPLC only data: 
 = threshold >= length(unique(gc_hplc_list[[1]]$NewFile))/3) resutled in 63 compounds. This produces PCA that can separate bottle caps, Food packaging, Fishing bait trays, (PolystyreneFW + Cigar tips) into 1 cluster and the rest.
 = threshold >= length(unique(gc_hplc_list[[1]]$NewFile))/2.5) resutled in 24 compounds. This produces PCA that can separate Cigar tips, (Food packaging + Plastic toy balls + Food containers + BOttle caps), (Fishing bait trays + PolystyFW), (PDS + Food wrappers), Ziploc bags,  Various plastic waste into 1 cluster.
