...
newmetal <- icp_list[[1]] %>% tidyr::pivot_wider(names_from = Feature, values_from = Values) %>% column_to_rownames(., var = "File")
...
for (row in 1:nrow(hplc)) {
  ...
  top_hplc[[hplc[row,]$Subcategory]] <- colnames(hplc)[order(as.numeric(hplc[row,]), decreasing = TRUE)[1:1000]]
}
...
ggplot(data = joined, aes(x = Subcategory, y = Values, fill = `Suspect Name`)) +
  ...
  #                                        ymax = mean + sd, group = Metal_component), 
  #               position = position_dodge(width = 0.9), width = 0.25) +
  labs(title = "Stacked Bar Plot", x = "Plastic products", y = "Normalized Peak Abundance (%)",
       fill = "Top HPLC features") +
  theme_minimal(base_size = 20) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
