...
# Blank samples 
# setwd("C:/Users/huyng/OneDrive - Toronto Metropolitan University/Microplastic/Microplastic-Fingerprinting/data/ATDGCMS/Blank_ATDGCMS")
...
### Research Question with Trace metal data
...
for (i in 1:length(excel_sheets(path = p))) {
  ...
  metal_list[[i]] <- dat %>% 
    select(., -c(Type)) %>%
    relocate(File, NewFile, Suspected_Polymer, Category, Subcategory, .before = 1) %>%
    pivot_longer(., cols = 6:ncol(.), names_to = "Metal_component", values_to = "Values") %>% 
    # Since each sample are run in triplicates, the mean or median of the 3 reps are calculated for each type of USE-..
    group_by(File, Metal_component) %>% 
    summarise(across(Values, stats::median)) # base::mean
}
```
...
factoextra::fviz_pca_biplot(res.pca,  
                            ...
                            labelsize = 6, 
                            ggtheme = ggplot2::theme_minimal(base_size = 15),
                            title = paste0("Trace metal samples grouping by samples ", 2))
```
...
writexl::write_xlsx(x = list_suspect_pol, path = "Wilcoxon_test_GC_data_by_Suspected_polymer_Missing-Observation.xlsx")
```
...
writexl::write_xlsx(x = suspect_pol, path = "Wilcoxon_test_GC_data_by_Suspected_Polymer_NA imputed.xlsx")
```
...
writexl::write_xlsx(x = sig_comp_gc, path = "Wilcoxon_test_GC_data_by_Category_Missing-Observation.xlsx")
```
...
writexl::write_xlsx(x = sig_comp_gc, path = "Wilcoxon_test_GC_data_by_Category_NA imputed.xlsx")
```
...
writexl::write_xlsx(x = sub_cat, path = "Wilcoxon_test_COMBINED GC_HPLC_by_Subcategory_NA imputed.xlsx")
```
...
writexl::write_xlsx(x = list_subcat, path = "Wilcoxon_test_GC_data_by_Subcategory_Missing-observation.xlsx")
```
...
writexl::write_xlsx(x = sub_cat, path = "Wilcoxon_test_GC_data_by_Subcategory_NA imputed.xlsx")
```
