...
combined_df1 <- rbind(df1, df_blank1) %>% arrange(RT)
...
input_df <- function(data) {
  # create sample df
  df_X_rq1 <- data %>%
    dplyr::select(File, collapsed_compound, Values) %>%
    ...
    tidyr::pivot_wider(names_from = File, values_from = Values) %>%
  ...
  Suspected_Polymer <- c()
  for (File in metadata_X_rq1$File) {
    Suspected_Polymer <- c(Suspected_Polymer, unique(data[which(data$File == File),]$Suspected_Polymer))
  }
  metadata_X_rq1$Suspected_Polymer <- Suspected_Polymer
  metadata_X_rq1 <- metadata_X_rq1 %>% tibble::column_to_rownames(., var = "File")
...
dim(PCAtools_mergePC)
```

### columns as "Suspected_Polymer"/"Category"/"Subcategory"
...
input_df <- function(data) {
  ...
  df_X_rq1 <- data %>%
    dplyr::select(File, 
                  Subcategory, # Category, # Suspected_Polymer,  Subcategory
                  collapsed_compound, 
                  Values) %>%
    # since we have multiple different values of the same compound in some samples, we summarize these values by taking the mean of them
    dplyr::group_by(
      Subcategory, # Category, # Suspected_Polymer, 
      collapsed_compound) %>%
    dplyr::summarise(across(Values, base::mean)) %>%
    tidyr::pivot_wider(names_from =  Subcategory, # Category, #Suspected_Polymer, Subcategory
                       values_from = Values) %>%
    tibble::column_to_rownames(., var = "collapsed_compound")
  ...
  # table for information (rows are sample IDs, columns are sample information)
  metadata_X_rq1 <- data.frame(Subcategory # Category # Suspected_Polymer Subcategory
                               = colnames(df_X_rq1))
  Subcategory2 <- metadata_X_rq1$Subcategory # Category  # Suspected_Polymer
...
stats <- gc_hplc_list[[1]] %>%
  ...
  group_by(File, collapsed_compound) %>%
  summarise(mean_value = mean(Values)) %>%
  pivot_wider(names_from = collapsed_compound, values_from = mean_value)
...
# Main loop
for (ele in 91:length(hca$height)) {
  list_hca <- map(1:7, function(min_obs) {
    t <- data.frame(group = cutree(hca_samp, h = ceiling(hca_samp$height[ele])))
    t <- rownames_to_column(t, var = "File")
    joined <- full_join(t, stats, by = "File")
    
    g_df <- map_dfr(3:ncol(joined), function(c) {
      satisfied_group <- unique(joined$group[sapply(joined[, c], function(x) sum(!is.na(joined[joined$group == x, c]))) > min_obs])
      if (length(satisfied_group) < 2) return(NULL)
      
      pairs <- combn(satisfied_group, 2)
      map_dfr(1:ncol(pairs), function(col) {
        p_1 <- pairs[1, col]
        p_2 <- pairs[2, col]
        pval_wilcox_test <- wilcox.test(joined[joined$group == p_1, c], joined[joined$group == p_2, c])$p.value
        pval_ks_test <- ks.test(joined[joined$group == p_1, c], joined[joined$group == p_2, c])$p.value
        data.frame(compound = names(joined)[c], comparison_pair = paste0(p_1, "&", p_2), pval_wilcox_test, pval_ks_test)
      })
    })
    
    g_df$adjusted_pvalue_holm <- p.adjust(g_df$pval_wilcox_test, method = "holm")
    filter(g_df, adjusted_pvalue_holm < 0.05) %>% arrange(adjusted_pvalue_holm)
  })
  
  # Output and update
  max_nrow <- max(map_int(list_hca, nrow))
  cat(sprintf("The max. number of significant compounds at optimal no. of obs is %d at HCA cut-off Height threshold of %f\n", max_nrow, hca$height[ele]))
  sig_comp[[count]] <- list_hca[[which.max(map_int(list_hca, nrow))]]
  count <- count + 1
}
```
...
hca <- hclust(stats::dist(as.matrix(hc_df), 
                         method = "canberra"))
...
for (ele in 1:length(hca$height)) {
  list_hca <- list()
  for (min_obs in 1:8){
    t <- as.data.frame(cutree(hca,  h = ceiling(hca$height[ele])))
    colnames(t) <- "group"
    t <- t %>% tibble::rownames_to_column(., var = "File")
    # Joining stats df and HCA-grouping clusters
    joined <- full_join(t, stats, by = "File")
    g_df <- data.frame(compound=character(), comparison_pair=character(), pval_wilcox_test=integer(), pval_ks_test=integer())
    # Loop through each collapsed_compound (as columns)
    for (c in 3:ncol(joined)) {
      satisfied_group <- c()
      # Loop through each grouping from HCA
      for (group in unique(joined$group)) {
        # get all group that has > min_obs
        if (sum(!is.na(joined[which(joined$group == group), c])) > min_obs) {
          satisfied_group <- c(satisfied_group, group)
        } else {
          next
        }
      }
      # IF there is only 1 product_cat have more than n obs, then go to the next column (aka. compound)
      if (length(satisfied_group) < 2) {
        next
      } else {
        # looping through the combinations of product categories 
        for (col in 1:ncol(utils::combn(satisfied_group, 2))) {
          # extract the combinations of product category pairs
          p_1 <- utils::combn(satisfied_group, 2)[1,col]
          p_2 <- utils::combn(satisfied_group, 2)[2,col]
          ...
          g_df[nrow(g_df) + 1,] <- c(colnames(joined)[c], 
                                     paste0(p_1, "&", p_2), 
                                     pval_wilcox_test, 
                                     pval_ks_test)
        }
    ...
    g_df$adjusted_pvalue_holm <- stats::p.adjust(g_df$pval_wilcox_test, method = "holm")
    
    list_hca[[min_obs]] <- g_df %>%
            filter(., adjusted_pvalue_holm < 0.05) %>%
            arrange(adjusted_pvalue_holm)
  }
  
  print(paste0("The max. number of significant compounds ","at the optimal no. of obs ", which.max(lapply(list_hca, nrow)), " at HCA cut-off Height threshold of ",hca$height[ele]," is ", nrow(list_hca[[which.max(lapply(list_hca, nrow))]])))
  print("=======================================================================")
  # Select the cutting threshold with min_obs obs that results in the most number of significant compounds
  sig_comp[[count]] <- list_hca[[which.max(lapply(list_hca, nrow))]]
  count <- count + 1
  print(list_hca[[which.max(lapply(list_hca, nrow))]])
}
```
...
sig_comp <-list()
count <- 1
for (ele in 26:length(hca$height)) { # first 15 heigt results in empty df
  list_hca <- list()
  for (min_obs in 1:8){
    t <- as.data.frame(cutree(hca,  h = ceiling(hca$height[ele])))
    colnames(t) <- "group"
...
writexl::write_xlsx(x = list_suspect_pol, path = "Wilcoxon_test_COMBINED GC_HPLC_by_Suspected_polymer.xlsx")
```
...
for (i in 1:length(gc_hplc_list)) {
  ...
  for (row in 1:nrow(sig_comp[[paste0("Grouping_", i)]])) {
    idx <- base::which(gc_hplc_list[[i]]$collapsed_compound == sig_comp[[paste0("Grouping_", i)]][row,]$comp)
...
writexl::write_xlsx(x = sig_comp, path = "Wilcoxon_test_COMBINED GC_HPLC_by_Category.xlsx")
```
...
for (i in 1:length(gc_hplc_list)) {
  ...
  for (min_obs in 1:10) {
    ...
    list_subcat[[min_obs]] <- subcat_df %>%
      filter(., adjusted_pvalue_holm < 0.1) %>% # 0.05
      arrange(adjusted_pvalue_holm)
...
writexl::write_xlsx(x = sig_comp, path = "Wilcoxon_test_COMBINED GC_HPLC_by_Subcategory.xlsx")
```

When we ran a loop to investigate the significant compounds that can be obtained from getting all product_cat that has \> 3 to 6 obs, \> 5 obs gave the highest amount of significant compound. Overall, it is not surprising that most of the significant compounds are pointing towards distinguishing Toys and Food contact materials, which, from the PCA result, are the two most separated clusters. 
