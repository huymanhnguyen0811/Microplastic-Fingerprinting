...
stats <- gc_hplc_list[[1]] %>%
      select(NewFile, collapsed_compound, Values) %>%
      group_by(NewFile, collapsed_compound) %>%
...
for (ele in 1:length(hca$height)) {
  list_hca <- map(1:10, function(min_obs) {
...
for (ele in 1:length(hca$height)) {
  ...
  for (min_obs in 1:8){
    ...
    colnames(t) <- "group"
    t <- t %>% tibble::rownames_to_column(., var = "NewFile")
