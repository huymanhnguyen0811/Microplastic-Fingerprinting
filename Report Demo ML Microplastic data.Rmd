...
comp_filter_ver2 <- function(data) {
...
# STEP 1.2B Filtering out chemical noise (aka. setting limit of observations) ---------------------------- 
...
# STEP 3: Normalizing data accordingly to different data frames of interest on with the one with positive Area values and extracting negative values to be used as LOD -----------------------------------
...
for (i in 1:length(excel_sheets(path = p))) {
  ...
  dat1 <- left_join(x=comp_normalized1, 
                    y = sampinfo, 
                    by = 'File')
  
  idx_list_filter1 <- comp_filter_ver2(dat1) 
  ...
  # HPLCTOFMS
  # joining data frame with sampinfo df
  dat2 <- left_join(x=comp_normalized2, 
                    y = sampinfo, 
                    by = 'File')
  
  idx_list_filter2 <- comp_filter_ver2(dat2)
  ...
  atdgcms <- shared_comp1 %>%
    ...
    select('File', 'Suspected_Polymer', 'Category', 'Subcategory', 'collapsed_compound', 'Percent_Area')
  ...
  hplctofms <- shared_comp2 %>%
    filter(., !is.na(collapsed_compound)) %>%
    select('File', 'Suspected_Polymer', 'Category', 'Subcategory', 'collapsed_compound', 'Percent_Height')
...
# A bi-plot =================
PCAtools::biplot(p,
                 lab = rownames(p$metadata),  # rownames(pcaobj$metadata)
...
plot(hclust(vegan::vegdist(as.matrix(hc_df), 
                           method = "canberra")),  # Since our data is continous -> canberra // manhattan
     # // aitchison // robust.aitchison
     # labels = FALSE, 
     hang = -1,
     main = "Combined GC - HPLC data")
```
...
#### Here, we use a non-parametric test, Wilcoxon test, because our data vioates the assumptions for other parametric test, as demonstrate below:
...
sig_comp <-list()
count <- 1
for (ele in 62:length(hca$height)) {
  list_hca <- list()
  for (min_obs in 1:7){
...
for (min_obs in 1:10) {
  ...
  list_suspect_pol[[min_obs]] <- sp_df %>%
    filter(., adjusted_pvalue_holm < 0.05) %>%
    arrange(adjusted_pvalue_holm)
...
for (i in 1:length(gc_hplc_list)) {
  ...
  for (min_obs in 1:10) {
    stats <- gc_hplc_list[[i]] %>%
      select(File, collapsed_compound, Category, Values) %>%
      # since we have duplicates with different values of the same compound in some samples, we summarize these values by taking the mean of them
  ...
  sig_comp[[i]] <- list_cat[[which.max(lapply(list_cat, nrow))]]
  print(sig_comp[[i]] %>% arrange(comp))
}
```
...
for (i in 1:length(gc_hplc_list)) {
  ...
  for (min_obs in 1:10) {
    stats <- gc_hplc_list[[1]] %>%
      select(File, collapsed_compound, Subcategory, Values) %>%
      # since we have duplicates with different values of the same compound in some samples, we summarize these values by taking the mean of them
      group_by(File, Subcategory, collapsed_compound) %>%
      summarise(across(Values, mean)) %>%
      pivot_wider(names_from = collapsed_compound, values_from = Values) %>%
      column_to_rownames(., var = "File")
  ...
  sig_comp[[i]] <- list_subcat[[which.max(lapply(list_subcat, nrow))]]
  print(sig_comp[[i]])
}
```
...
rfsrc.result <- function(dat, split.ratio) {
  ...
  # Prediction results
  pred_res <- predict(rf, newdata = plastic_tst, type = "prob")$predicted
...
for (gc in gc_list) {
  ...
  for (r in 1:nrow(df)) {
    df[r, which(base::is.na(df[r,]))] <- runif(length(which(base::is.na(df[r,]))),
                                               min = sort(gc$Values)[1],
                                               max = sort(gc$Values)[2])
  }
