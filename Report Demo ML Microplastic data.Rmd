...
newmetal <- metal_list[[1]] %>% tidyr::pivot_wider(names_from = Metal_component, values_from = Values) %>% column_to_rownames(., var = "File")
...
topmetal_list <- list()
i <- 1
for (file in unique(metal_list[[1]]$File)) {
  topmetal_list[[i]] <- metal_list[[1]] %>% filter(., File %in% file) %>% filter(., Metal_component %in% top_metal[[i]])
  i <- i + 1
}

top_metal_plot <- bind_rows(topmetal_list)
...
ggplot(data = top_metal_plot, aes(x = File, y = Values, fill = Metal_component)) +
  geom_bar(stat = "identity") +
  # geom_errorbar(data = data_summary, aes(x = NewFile, ymin = mean - sd, 
  #                                        ymax = mean + sd, group = Metal_component), 
  #               position = position_dodge(width = 0.9), width = 0.25) +
  labs(title = "Stacked Bar Plot", x = "Sample", y = "Value",
       fill = "Top trace metal components") +
  theme_minimal(base_size = 20) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
  
```

#### 2.1 Among the top25 trace metal, if the metal is shared among different samples, is there a significant difference in the concentration of these trace metals?

```{r class.source = 'fold-hide', echo = FALSE, message = FALSE, warning = FALSE}

```

#### 2.2 Among the top25 trace metal, what are the metal that uniquely occur in just 1 sample?

```{r class.source = 'fold-hide', echo = FALSE, message = FALSE, warning = FALSE}
```

### Combining Trace Metal data with GC&HPLCdata - TBA

```{r class.source = 'fold-hide', echo = FALSE, message = FALSE, warning = FALSE}
# MAtch sample name with combined GC/HPLC
for (i in gc_hplc_list) {
  full_join(x = metal_list[[i]],
            y = gc_hplc_list[[i]] %>% 
              select(., -c(m.z, RT)), by = c("Suspected_Polymer", "Category", "Subcategory"))
}
```
...
#### Examining different threshold for minimum number of samples that a compound must occur in.

For HPLC only data, threshold \>= length(unique(hplc_list[[1]]\$NewFile))/2) produces PCA that can separate plastic ball into 1 cluster and PBBC into another clusters (with some FPW mixed up in it).

For GC only data, threshold \>= length(unique(gc_list[[1]]\$NewFile))/2) produces PCA that separates plastic drinking straw (USSB-09, which is the only PDS_USSB that made out of Corn material) and the rest.

For combined GC/HPLC only data, threshold \>= length(unique(gc_hplc_list[[1]]\$NewFile))/4) produces PCA that can separate plastic ball into 1 cluster and the rest.

```{r , echo=FALSE, warning = FALSE, message=FALSE}
# comp_vec <- c()
# for (comp in unique(gc_hplc_list[[1]]$collapsed_compound)) {
#   idx <- which(gc_hplc_list[[1]]$collapsed_compound == comp)
#   if (length(unique(gc_hplc_list[[1]][idx,]$NewFile)) >= (length(unique(gc_hplc_list[[1]]$NewFile))/4)) {
#     comp_vec <- c(comp_vec, comp)
#   }
# }
```
...
```{r , echo=FALSE, warning = FALSE, message=FALSE}
df_pca <- gc_list[[1]] %>%
  # filter(., collapsed_compound %in% comp_vec) %>%
  dplyr::select(NewFile, 
                collapsed_compound, 
                Values) %>%
  # since we have multiple different values of the same compound in some samples, we summarize these values by taking the mean of them
  dplyr::group_by(
    NewFile, # Category, # Suspected_Polymer, 
    collapsed_compound) %>%
  dplyr::summarise(across(Values, base::mean)) %>%
...
For HPLC only data, threshold \>= length(unique(hplc_list[[1]]\$NewFile))/1.5) produces PCA that can separate plastic ball, food packaging, bottle caps, fish trays, (Polystyrene food waste + Cigar tips) into 1 cluster and the rest.

For GC only data, threshold \>= length(unique(gc_list[[1]]\$NewFile))/2) resulted in 7 compounds, This produces PCA that separates plastic drinking straw, PDS, PB, Drink bottles, Balloons, Cigar Tips, Bottle caps (closer to plastic cups) and the rest.

For combined GC/HPLC only data: = threshold \>= length(unique(gc_hplc_list[[1]]$NewFile))/3) resutled in 63 compounds. This produces PCA that can separate bottle caps, Food packaging, Fishing bait trays, (PolystyreneFW + Cigar tips) into 1 cluster and the rest. = threshold >= length(unique(gc_hplc_list[[1]]$NewFile))/2.5) resutled in 24 compounds. This produces PCA that can separate Cigar tips, (Food packaging + Plastic toy balls + Food containers + BOttle caps), (Fishing bait trays + PolystyFW), (PDS + Food wrappers), Ziploc bags, Various plastic waste into 1 cluster.

```{r , echo=FALSE, warning = FALSE, message=FALSE}
comp_vec <- c()
for (comp in unique(gc_hplc_list[[1]]$collapsed_compound)) {
  idx <- which(gc_hplc_list[[1]]$collapsed_compound == comp)
  if (length(unique(gc_hplc_list[[1]][idx,]$NewFile)) >= (length(unique(gc_hplc_list[[1]]$NewFile))/2.5)) {
    comp_vec <- c(comp_vec, comp)
  }
}
```
...
##### "Category" --  4 different categorization schemes
...
```{r , echo=FALSE, warning = FALSE, message=FALSE}
comp_vec <- c()
for (comp in unique(hplc_list[[1]]$collapsed_compound)) {
  idx <- which(hplc_list[[1]]$collapsed_compound == comp)
  if (length(unique(hplc_list[[1]][idx,]$NewFile)) >= (length(unique(hplc_list[[1]]$NewFile))/3)) {
    comp_vec <- c(comp_vec, comp)
  }
}
```
...
p1 <- paste0(getwd(), "/Wilcoxon test results/Wilcoxon_test_Combined GC-HPLC_by_Robust Aichitson_HCA clustering result_CO151-162_06Dec2023.xlsx")
p2 <- paste0(getwd(), "/Wilcoxon test results/Wilcoxon_test_Combined GC-HPLC_by_Robust Aichitson_HCA clustering result_CO161-172_06Dec2023.xlsx")

sheet_names1 <- excel_sheets(path = p1)
sus_pol_combined1 <- list()

for (sheet in sheet_names1) {
  sus_pol_combined1[[sheet]] <- read_excel(path = p1, sheet = sheet)
} 

sheet_names2 <- excel_sheets(path = p2)
sus_pol_combined2 <- list()

for (sheet in sheet_names2) {
  sus_pol_combined2[[sheet]] <- read_excel(path = p2, sheet = sheet)
}

all <- c(sus_pol_combined1, sus_pol_combined2)
...
# Main loop
for (ele in 50:length(hca$height)) {
  t <- data.frame(group = cutree(hca, h = ceiling(hca$height[ele]))) %>% tibble::rownames_to_column(., var = "NewFile")
  joined <- full_join(t, stats, by = "NewFile")
...
```{r class.source = 'fold-hide', echo=FALSE, warning = FALSE, message=FALSE}
setwd("C:/Users/huyng/OneDrive - Toronto Metropolitan University/Microplastic/Microplastic-Fingerprinting/Wilcoxon test results")

files <- list.files(pattern = '*.xlsx') %>%
  .[str_detect(., "Wilcoxon_test_HPLC_by_Robust Aichitson_HCA clustering result_CO")]

sus_pol_combined <- list()
for (file in files) {
  print(file)
  sheet_names <- excel_sheets(path = paste0(getwd(), "/", file))
  for (sheet in sheet_names) {
    sus_pol_combined[[sheet]] <- read_excel(path = file, sheet = sheet)
  }
}
```
...
###### with Missing observation method - DONE
...
###### with NA imputation method - DONE

[UPdate 15th Dec 2023]: When we impute the missing data with 0 instead of stochaistic LOD, we have shit load more significant compounds (1095 versus. ~100)

```{r class.source = 'fold-hide', echo=FALSE, warning = FALSE, message=FALSE}
stats <- gc_hplc_list[[1]] %>%
  select(File, collapsed_compound, Suspected_Polymer, Values) %>%
  # since we have duplicates with different values of the same compound in some samples, we summarize these values by taking the mean of them
  group_by(File, Suspected_Polymer, collapsed_compound) %>%
  summarise(across(Values, mean)) %>%
  pivot_wider(names_from = collapsed_compound, values_from = Values) %>%
...
writexl::write_xlsx(x = suspect_pol, path = "Wilcoxon_test_COMBINED GC_HPLC_by_Suspected_Polymer_NA imputed_15thDec2023.xlsx")
```
...
View(c(unique_values_list1, unique_values_list2))
...
[Update 15th Dec 2023]: The dimension of the stats only 64 rows, which means byh updating the Blank files from Davie Jones, we effectively remove 12 samples from the pool of samples (previously 76 samples in total; now 64).
...
for (min_obs in 1:10) {
  ...
  print(list_suspect_pol[[paste0("Min_obs_", min_obs)]])
  ...
  mean_mz <- c()
  for (row in 1:nrow(list_suspect_pol[[paste0("Min_obs_", min_obs)]])) {
    idx <- base::which(gc_list[[1]]$collapsed_compound == list_suspect_pol[[paste0("Min_obs_", min_obs)]][row,]$comp)
...
writexl::write_xlsx(x = list_suspect_pol, path = "Wilcoxon_test_GC_data_by_Suspected_polymer_Missing-Observation_15Dec2023.xlsx")
```

###### with NA imputation method - DONE

[UPdate 15th Dec 2023]: Note that the Total percentage of each file (aka. row) in the stats data frame is not = 100% because at the last step of the data processing, we remove all the unique compounds that only appear in that one sample.s
...
if (dim(suspect_pol)[1] == 0) {
  ...
  mean_mz <- c()
  for (row in 1:nrow(suspect_pol)) {
    idx <- base::which(gc_list[[1]]$collapsed_compound == suspect_pol[row,]$comp)
...
writexl::write_xlsx(x = suspect_pol, path = "Wilcoxon_test_GC_data_by_Suspected_Polymer_NA imputed_15Dec2023.xlsx")
```

###### Summary of significant compounds - DONE

```{r class.source = 'fold-hide', echo=FALSE, warning = FALSE, message=FALSE}
p1 <- paste0(getwd(), "/Wilcoxon test results/Wilcoxon_test_GC_data_by_Suspected_polymer_Missing-Observation.xlsx")
p2 <- paste0(getwd(), "/Wilcoxon test results/Wilcoxon_test_GC_data_by_Suspected_Polymer_NA imputed.xlsx")
...
writexl::write_xlsx(x = all_unique, path = "Wilcoxon_test_GC_data_by_Suspected_Polymer_all_significant compounds.xlsx")
```
...
writexl::write_xlsx(x = list_suspect_pol, path = "Wilcoxon_test_HPLC_data_by_Suspected_polymer_Missing-Observation.xlsx")
```
...
if (dim(suspect_pol)[1] == 0) {
  ...
  mean_mz <- c()
  for (row in 1:nrow(suspect_pol)) {
    idx <- base::which(hplc_list[[1]]$collapsed_compound == suspect_pol[row,]$comp)
...
writexl::write_xlsx(x = suspect_pol, path = "Wilcoxon_test_HPLC_data_by_Suspected_Polymer_NA imputed.xlsx")
```

###### Summary of significant compounds - DONE

```{r class.source = 'fold-hide', echo=FALSE, warning = FALSE, message=FALSE}
p1 <- paste0(getwd(), "/Wilcoxon test results/Wilcoxon_test_HPLC_data_by_Suspected_polymer_Missing-Observation.xlsx")
p2 <- paste0(getwd(), "/Wilcoxon test results/Wilcoxon_test_HPLC_data_by_Suspected_Polymer_NA imputed.xlsx")
...
writexl::write_xlsx(x = all_unique, path = "Wilcoxon_test_HPLC_data_by_Suspected_Polymer_all_significant compounds.xlsx")
```

##### Metal
[Update 18th Dec 2023]: No significant compounds to distinguish between different Polymer type of different plastic products
```{r class.source = 'fold-hide', echo=FALSE, warning = FALSE, message=FALSE}
stats <- metal_list[[1]] %>%
  select(File, Metal_component , Suspected_Polymer, Values) %>%
  # since we have duplicates with different values of the same compound in some samples, we summarize these values by taking the mean of them
  group_by(File, Suspected_Polymer, Metal_component) %>%
  summarise(across(Values, mean)) %>%
  pivot_wider(names_from = Metal_component , values_from = Values) %>%
  column_to_rownames(., var = "File")
...
# Loop through each Metal_component  (as columns)
for (c in 2:ncol(stats)) {
  # looping through the combinations of product categories 
  for (col in 1:ncol(utils::combn(unique(stats$Suspected_Polymer), 2))) {
    # extract the combinations of product category pairs
    p_1 <- utils::combn(unique(stats$Suspected_Polymer), 2)[1,col]
    p_2 <- utils::combn(unique(stats$Suspected_Polymer), 2)[2,col]
    # calculating the p-value between each product category pair 
    pval_wilcox_test <- wilcox.test(stats[which(stats$Suspected_Polymer == p_1), c], 
                                    stats[which(stats$Suspected_Polymer == p_2), c])$p.value
    
    # assigning row information
    comp_list <- c(comp_list, colnames(stats)[c])
    pair_list <- c(pair_list, paste0(p_1, " & ", p_2))
    pval_list <- c(pval_list, pval_wilcox_test)
  }
}

cat_df <- data.frame(Compound = comp_list, Pair = pair_list, pval = pval_list)
cat_df$adjusted_pvalue_holm <- stats::p.adjust(cat_df$pval, method = "holm")

sig_comp[[paste0("Grouping_", i)]] <- cat_df %>%
  filter(., adjusted_pvalue_holm < 0.05) %>%
  arrange(adjusted_pvalue_holm)

print(sig_comp[[paste0("Grouping_", i)]] %>% arrange(comp))

writexl::write_xlsx(x = sig_comp, path = "Wilcoxon_test_Trace_Metal_data_by_Polymer_type_18thDec23.xlsx")
```
...
writexl::write_xlsx(x = sig_comp_gchplc, path = "Wilcoxon_test_COMBINED GC_HPLC_by_Category_Missing-Observation_05Dec2023.xlsx")
```
...
writexl::write_xlsx(x = sig_comp_gchplc, path = "Wilcoxon_test_COMBINED GC_HPLC_by_Category_NA imputed_05Dec2023.xlsx")
```

###### Summary of significant compounds - DONE

```{r class.source = 'fold-hide', echo=FALSE, warning = FALSE, message=FALSE}
p1 <- paste0(getwd(), "/Wilcoxon test results/Wilcoxon_test_COMBINED GC_HPLC_by_Category_Missing-Observation_05Dec2023.xlsx")
p2 <- paste0(getwd(), "/Wilcoxon test results/Wilcoxon_test_COMBINED GC_HPLC_by_Category_NA imputed_05Dec2023.xlsx")
...
for (i in 1:length(gc_list)) {
  ...
  if (dim(sig_comp_gc[[paste0("Grouping_", i)]])[1] == 0) {
    ...
    for (row in 1:nrow(sig_comp_gc[[paste0("Grouping_", i)]])) {
      idx <- base::which(gc_list[[i]]$collapsed_compound == sig_comp_gc[[paste0("Grouping_", i)]][row,]$comp)
...
writexl::write_xlsx(x = sig_comp_gc, path = "Wilcoxon_test_GC_data_by_Category_Missing-Observation_05Dec2023.xlsx")
```
...
writexl::write_xlsx(x = sig_comp_gc, path = "Wilcoxon_test_GC_data_by_Category_NA imputed_05Dec2023.xlsx")
```

###### Summary of significant compounds - DONE

```{r class.source = 'fold-hide', echo=FALSE, warning = FALSE, message=FALSE}
p1 <- paste0(getwd(), "/Wilcoxon test results/Wilcoxon_test_GC_data_by_Category_Missing-Observation_05Dec2023.xlsx")
p2 <- paste0(getwd(), "/Wilcoxon test results/Wilcoxon_test_GC_data_by_Category_NA imputed_05Dec2023.xlsx")
...
for (i in 1:length(hplc_list)) {
  ...
  if (dim(sig_comp_hplc[[paste0("Grouping_", i)]])[1] == 0) {
    ...
    for (row in 1:nrow(sig_comp_hplc[[paste0("Grouping_", i)]])) {
      idx <- base::which(hplc_list[[i]]$collapsed_compound == sig_comp_hplc[[paste0("Grouping_", i)]][row,]$comp)
...
writexl::write_xlsx(x = sig_comp_hplc, path = "Wilcoxon_test_HPLC_data_by_Category_Missing-Observation.xlsx")
```
...
writexl::write_xlsx(x = sig_comp_hplc, path = "Wilcoxon_test_HPLC_data_by_Category_NA imputed.xlsx")
```
...
##### Trace Metal
Metal data does not have any NA so missing values method are not applied.

[Update 18th Dec 2023]: No significant compound found
```{r class.source = 'fold-hide', echo=FALSE, warning = FALSE, message=FALSE}
sig_comp <- list()

for (i in 1:length(metal_list)) {
  stats <- metal_list[[i]] %>%
    select(File, Metal_component , Category, Values) %>%
    # since we have duplicates with different values of the same compound in some samples, we summarize these values by taking the mean of them
    group_by(File, Category, Metal_component) %>%
    summarise(across(Values, mean)) %>%
    pivot_wider(names_from = Metal_component , values_from = Values) %>%
    column_to_rownames(., var = "File")
  ...
  for (c in 2:ncol(stats)) {
    # looping through the combinations of product categories 
    for (col in 1:ncol(utils::combn(unique(stats$Category), 2))) {
      # extract the combinations of product category pairs
      p_1 <- utils::combn(unique(stats$Category), 2)[1,col]
      p_2 <- utils::combn(unique(stats$Category), 2)[2,col]
      # calculating the p-value between each product category pair 
      pval_wilcox_test <- wilcox.test(stats[which(stats$Category == p_1), c], 
                                      stats[which(stats$Category == p_2), c])$p.value
...
writexl::write_xlsx(x = sig_comp, path = "Wilcoxon_test_Trace_Metal_data_by_Category_NA imputed_18thDec23.xlsx")
```
...
writexl::write_xlsx(x = list_subcat, path = "Wilcoxon_test_COMBINED GC_HPLC_by_Subcategory_Missing-observation_05Dec2023.xlsx")
```
...
writexl::write_xlsx(x = sub_cat, path = "Wilcoxon_test_COMBINED GC_HPLC_by_Subcategory_NA imputed_08Dec2023.xlsx")
```
...
for (min_obs in 1:10) {
  ...
  mean_mz <- c()
  for (row in 1:nrow(list_subcat[[paste0("Min_obs_", min_obs)]])) {
    idx <- base::which(gc_list[[1]]$collapsed_compound == list_subcat[[paste0("Min_obs_", min_obs)]][row,]$comp)
    mean_rt <- c(mean_rt, round(mean(gc_list[[1]][idx, ]$RT), 3))
    mean_mz <- c(mean_mz, round(mean(gc_list[[1]][idx, ]$m.z), 3))
  }
...
writexl::write_xlsx(x = list_subcat, path = "Wilcoxon_test_GC_data_by_Subcategory_Missing-observation_08Dec2023.xlsx")
```
...
for (r in 1:nrow(stats)) {
  stats[r, which(base::is.na(stats[r,]))] <- stats::runif(length(which(base::is.na(stats[r,]))),
                                                          min = sort(gc_list[[1]]$Values)[1]/10000,
                                                          max = sort(gc_list[[1]]$Values)[2]/10000)
}
...
if (dim(sub_cat)[1] == 0) {
  ...
  mean_mz <- c()
  for (row in 1:nrow(sub_cat)) {
    idx <- base::which(gc_list[[1]]$collapsed_compound == sub_cat[row,]$comp)
    mean_rt <- c(mean_rt, mean(gc_list[[1]][idx, ]$RT))
    mean_mz <- c(mean_mz, mean(gc_list[[1]][idx, ]$m.z))
  }
...
writexl::write_xlsx(x = sub_cat, path = "Wilcoxon_test_GC_data_by_Subcategory_NA imputed_08Dec2023.xlsx")
```
...
for (min_obs in 1:10) {
  ...
  mean_mz <- c()
  for (row in 1:nrow(list_subcat[[paste0("Min_obs_", min_obs)]])) {
    idx <- base::which(hplc_list[[1]]$collapsed_compound == list_subcat[[paste0("Min_obs_", min_obs)]][row,]$comp)
    mean_rt <- c(mean_rt, round(mean(hplc_list[[1]][idx, ]$RT), 3))
    mean_mz <- c(mean_mz, round(mean(hplc_list[[1]][idx, ]$m.z), 3))
  }
...
writexl::write_xlsx(x = list_subcat, path = "Wilcoxon_test_HPLC_data_by_Subcategory_Missing-observation.xlsx")
```
...
if (dim(sub_cat)[1] == 0) {
  ...
  mean_mz <- c()
  for (row in 1:nrow(sub_cat)) {
    idx <- base::which(hplc_list[[1]]$collapsed_compound == sub_cat[row,]$comp)
    mean_rt <- c(mean_rt, mean(hplc_list[[1]][idx, ]$RT))
    mean_mz <- c(mean_mz, mean(hplc_list[[1]][idx, ]$m.z))
  }
...
writexl::write_xlsx(x = sub_cat, path = "Wilcoxon_test_HPLC_data_by_Subcategory_NA imputed.xlsx")
```
...
#### By Trace Metal - DONE
[Update 18th Dec 2023]: No significant compounds to distinguish between Subcategory (in 4 Subcategory grouping)

```{r class.source = 'fold-hide', echo=FALSE, warning = FALSE, message=FALSE}
stats <- metal_list[[1]] %>%
  select(File, Metal_component , Subcategory, Values) %>%
  # since we have duplicates with different values of the same compound in some samples, we summarize these values by taking the mean of them
  group_by(File, Subcategory, Metal_component) %>%
  summarise(across(Values, mean)) %>%
  pivot_wider(names_from = Metal_component , values_from = Values) %>%
  column_to_rownames(., var = "File")
...
writexl::write_xlsx(x = sig_comp, path = "Wilcoxon_test_Trace_Metal_data_by_Subcategory_18thDec23.xlsx")
```

## Matching Wilcoxon significant compounds from HPLC data with suspect screening from EF

```{r class.source = 'fold-hide', echo=FALSE, warning = FALSE, message=FALSE}
sus_scr <- readxl::read_xlsx(path = paste0(getwd(), "/PMT Suspect Screening Plastics Results EF.xlsx"), skip = 1)
...
# Find in all_uniquec the compounds that match with suspects in all_unique with molecualr ions in the range of +- 0.0005

for (row in all_unique) {
  # Extract m/z
  all_unique$mean_mz >= (all_unique$mean_mz)
}
```
#### 2.2 Category
#### 2.3 Subcategory
...
caret.multinomlog.result <- function(df, split.ratio){
  set.seed(1234)
  plastic_idx <- caret::createDataPartition(df$Category, p = split.ratio, list = F)
  ...
  caret::confusionMatrix(max_columns, plastic_tst$Category)
