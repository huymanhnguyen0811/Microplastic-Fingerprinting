...
for (ele in 1:length(hca$height)) {
  ...
  # Output and update
  max_nrow <- max(map_int(list_hca, nrow))
  cat(sprintf("Only GC data_The max. number of significant compounds at optimal no. of obs is %d at HCA cut-off Height threshold of %f\n", max_nrow, hca$height[ele]))
  sig_comp[[count]] <- list_hca[[which.max(map_int(list_hca, nrow))]]
  print(sig_comp[[count]])
  count <- count + 1
}
...
for (r in 1:nrow(hc_df)) {
  hc_df[r, which(base::is.na(hc_df[r,]))] <- stats::runif(length(which(base::is.na(hc_df[r,]))),
                                                          min = sort(hplc_list[[1]]$Values)[1],
                                                          max = sort(hplc_list[[1]]$Values)[2])
}
...
for (ele in 1:length(hca$height)) {
  ...
  # Output and update
  max_nrow <- max(map_int(list_hca, nrow))
  cat(sprintf("Only HPLC data_The max. number of significant compounds at optimal no. of obs is %d at HCA cut-off Height threshold of %f\n", max_nrow, hca$height[ele]))
  sig_comp[[count]] <- list_hca[[which.max(map_int(list_hca, nrow))]]
  print(sig_comp[[count]])
  count <- count + 1
}
