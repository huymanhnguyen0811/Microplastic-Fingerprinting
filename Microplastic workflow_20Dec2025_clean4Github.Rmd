...
library(vegan)
library(readxl)
library(tidyverse) # Loads dplyr, tidyr, purrr, etc.
library(data.table)
library(writexl)
library(grid)
library(gridExtra)
...
source("Helper Functions for Computational Workflow.R")
```
...
path_batch0 <- "./data/HPLCTOFMS/EF_Non-target data_Batch 0"
path_batch1 <- "./data/HPLCTOFMS/EF_Non-target data_Batch 1"
path_batch2 <- "./data/HPLCTOFMS/EF_Non-target data_Batch 2"

# Function to safely list files and read them
read_hplc_batch <- function(dir_path, batch_num) {
  
  # Get Sample Files (full path needed for read_xls)
  files_sample <- list.files(path = dir_path, pattern = '*.xls', full.names = TRUE) %>%
    .[!str_detect(., "Blank")]
  
  # Get Blank Files
  files_blank <- list.files(path = dir_path, pattern = '*.xls', full.names = TRUE) %>%
    .[str_detect(., "Blank")]
  
  # Read and combine
  combined <- dplyr::bind_rows(
    purrr::map(files_sample, read_xls, skip = 1), 
    purrr::map(files_blank, read_xls, skip = 1)
  ) %>%
