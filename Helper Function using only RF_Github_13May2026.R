...
process_and_filter <- function(
  ...
  # Metadata columns
  meta_cols <- c(class_col, "technique", "Source", "Polymer")
...
find_best_impute_normalize <- function(
  ...
  for (imp_name in imputation_methods) {
    for (norm_name in normalization_methods) {
      ...
      X_norm_num <- X_norm[, sapply(X_norm, is.numeric), drop = FALSE]
      
      # Remove zero-variance columns
      keep_cols <- names(which(
        sapply(X_norm_num, function(col)
          sd(col, na.rm = TRUE) > 0)
      ))
      
      X_num <- X_num[, keep_cols, drop = FALSE]
      X_norm_num <- X_norm_num[, keep_cols, drop = FALSE]
      
      orig_corr <- tryCatch(
        cor(X_num, use = "pairwise.complete.obs"),
        error = function(e) NULL
      )
      
      imp_corr <- tryCatch(
        cor(X_norm_num, use = "pairwise.complete.obs"),
        error = function(e) NULL
      )
...
data_fusion <- function(df_input, source_feats, class_col = "Plastic_type") {
  ...
  df_wide <- df_input %>%
    ...
    tibble::column_to_rownames(var = "File") %>%
    dplyr::relocate(
      any_of(c(class_col, "technique", "Source", "Polymer", "Subcategory")),
      .before = 1
    )
  
  # Metadata columns
  meta_cols <- intersect(
    c(class_col, "technique", "Source", "Polymer", "Subcategory"),
    colnames(df_wide)
  )
  ...
  missing_cols <- setdiff(requested_cols, colnames(df_wide))
  ...
  output <- df_wide[, existing_cols, drop = FALSE]
  return(output)
}
...
export_rf_results <- function(results, output_dir = ".", fold, prefix) {
  ...
  # Feature summary ------------------------------------------------------------
  #### Pairwise
  write.csv(results[[fold]]$sig_selected_feats, 
            file.path(output_dir, paste0(prefix, "_pairwise_sig_features.csv")), 
            row.names = FALSE)
  #### RFA
  write.csv(results[[fold]]$rfa_selected_feats, 
            file.path(output_dir, paste0(prefix, "_rfa_features.csv")), 
            row.names = FALSE)
  #### Top 100/50/25/10 features
  write.csv(results[[fold]]$top_importance_results$top_100$selected_features, 
            file.path(output_dir, paste0(prefix, "_top 100 RF_features.csv")), 
            row.names = FALSE)
  write.csv(results[[fold]]$top_importance_results$top_50$selected_features, 
            file.path(output_dir, paste0(prefix, "_top 50 RF_features.csv")), 
            row.names = FALSE)
  write.csv(results[[fold]]$top_importance_results$top_25$selected_features, 
            file.path(output_dir, paste0(prefix, "_top 25 RF_features.csv")), 
            row.names = FALSE)
  write.csv(results[[fold]]$top_importance_results$top_10$selected_features, 
            file.path(output_dir, paste0(prefix, "_top 10 RF_features.csv")), 
            row.names = FALSE)
  
  # Classification MCC score ------------------------------------------------------------
  metrics_df <- data.frame(
    metric = c("MCC (All features)", "MCC (Pairwise)", "MCC (RFA)", 
               "MCC (top 100)", "MCC (Top 50)", "MCC (Top 25)", "MCC (Top 10)"),
    value = c(results[[fold]]$all_features_mcc, 
              results[[fold]]$mcc_sig, 
              results[[fold]]$mcc_rfa,
              results[[fold]]$top_importance_results$top_100$eval_metrics_top$MCC_Multiclass,
              results[[fold]]$top_importance_results$top_50$eval_metrics_top$MCC_Multiclass,
              results[[fold]]$top_importance_results$top_25$eval_metrics_top$MCC_Multiclass,
              results[[fold]]$top_importance_results$top_10$eval_metrics_top$MCC_Multiclass)
  )
  ...
  # Save plots ------------------------------------------------------------
  #### All features
  if (!is.null(results[[fold]]$conf_mat_all_feats)) {
    ggplot2::ggsave(file.path(output_dir, paste0(prefix, "_all_feats_confusion_matrix.png")),
                    results[[fold]]$conf_mat_all_feats, width = 15, height = 8, dpi = 300, units = "in")
  }
  #### Pairwise
  if (!is.null(results[[fold]]$conf_mat_sig)) {
    ggplot2::ggsave(file.path(output_dir, paste0(prefix, "_pairwise_confusion_matrix.png")),
                    results[[fold]]$conf_mat_sig, width = 15, height = 8, dpi = 300, units = "in")
  }
  #### RFA
  if (!is.null(results[[fold]]$conf_mat_rfa)) {
    ggplot2::ggsave(file.path(output_dir, paste0(prefix, "_RFA_confusion_matrix.png")),
                    results[[fold]]$conf_mat_rfa, width = 15, height = 8, dpi = 300, units = "in")
  }
  #### Top 100/50/25/10 features
  if (!is.null(results[[fold]]$top_importance_results$top_100$conf_mat_top)) {
    ggplot2::ggsave(file.path(output_dir, paste0(prefix, "_top 100 RF_confusion_matrix.png")),
                    results[[fold]]$top_importance_results$top_100$conf_mat_top, width = 15, 
                    height = 8, dpi = 300, units = "in")
  }
  if (!is.null(results[[fold]]$top_importance_results$top_50$conf_mat_top)) {
    ggplot2::ggsave(file.path(output_dir, paste0(prefix, "_top 50 RF_confusion_matrix.png")),
                    results[[fold]]$top_importance_results$top_50$conf_mat_top, width = 15, 
                    height = 8, dpi = 300, units = "in")
  }
  if (!is.null(results[[fold]]$top_importance_results$top_25$conf_mat_top)) {
    ggplot2::ggsave(file.path(output_dir, paste0(prefix, "_top 25 RF_confusion_matrix.png")),
                    results[[fold]]$top_importance_results$top_25$conf_mat_top, width = 15,
                    height = 8, dpi = 300, units = "in")
  }
  if (!is.null(results[[fold]]$top_importance_results$top_10$conf_mat_top)) {
    ggplot2::ggsave(file.path(output_dir, paste0(prefix, "_top 10 RF_confusion_matrix.png")),
                    results[[fold]]$top_importance_results$top_10$conf_mat_top, width = 15, 
                    height = 8, dpi = 300, units = "in")
  }
}