...
1. The specs that the author used for running this workflow is: .

2. Print name + version of R package to be stored as .txt on Github page.
...
# Take original data from process_and_filter for quality assurance report ---------
...
# B. Extract Data for Calculation
df_final <- gc_hplc_icp_combined_SB_ENV_shared_cols_ntr_clean_13Dec2025
df_orig  <- results$original_data
...
# --- 2. Percentage of missing values after feature reduction (%) ---
# We calculate this ONLY on the feature columns of the filtered dataset
if(!identical(setdiff(names(gc_hplc_icp_combined_SB_ENV_shared_cols_ntr_clean_13Dec2025), meta_cols), character(0))) {
  vals_final <- df_final[, feats_final]
  pct_missing <- (sum(is.na(vals_final)) / prod(dim(vals_final))) * 100
  cat(paste0("2. Percentage of missing values after feature reduction: ", round(pct_missing, 2), "%\n"))
} else {
