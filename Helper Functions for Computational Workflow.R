...
process_and_filter <- function(df_input, 
                               ...
                               threshold = 0.8, 
                               filter_mode = "regular80") { # "regular", "class", or "inhouse"
  ...
  if (filter_mode == "regular80") {
    ...
    stop("Invalid filter_mode. Choose 'regular80', 'modified80', or 'inhouse'.")
  }
...
impute_train_apply <- function(X_train, X_test, imp_name, k = 5){
  ...
  if(imp_name == "random_forest_imputation"){
    ...
    for(j in 1:ncol(train_filled)) {
      ...
      if(all(is.na(train_filled[,j]))) {
        ...
        train_filled[is.na(train_filled[,j]), j] <- mean(train_filled[,j], na.rm=TRUE)
      }
    }
    ...
    for(target in na_cols){
      ...
      if(any(idx_train)){
        preds_train <- predict(rf_model, data = train_filled[idx_train, ])$predictions
      ...
      if(any(idx_test)){
        preds_test <- predict(rf_model, data = test_filled[idx_test, ])$predictions
  ...
  if(imp_name == "knn_imputation"){
    ...
    if(!getDoParRegistered()) {
      message("Tip: Register a parallel backend (doParallel) to speed up KNN imputation.")
    }
    ...
    if(length(zero_var_cols) > 0) {
      cat(sprintf("\n[Auto-Fix] Dropping %d zero-variance cols before KNN (e.g., %s...)\n", 
                  length(zero_var_cols), zero_var_cols[1]))
    ...
    if(ncol(X_train) == 0) {
      warning("All columns had zero variance and were dropped. Returning empty datasets.")
